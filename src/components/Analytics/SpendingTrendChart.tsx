...
import { format, parseISO, isWithinInterval, startOfMonth, endOfDay } from 'date-fns';

const formatDateForInput = (date: Date): string => format(date, 'yyyy-MM-dd');

export const SpendingTrendChart = ({ finances }: { finances: FinanceEntity[] }) => {
  const [startDate, setStartDate] = useState<string>(() => formatDateForInput(startOfMonth(new Date())));
  const [endDate, setEndDate] = useState<string>(() => formatDateForInput(endOfDay(new Date())));
  ...
  return (
    <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', minHeight: 400 }}>
      <Typography variant="h6" color="primary" gutterBottom>Spending Trends</Typography>
      ...
      {filteredAndAggregatedData.length === 0 ? (
        <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: 200 }}>
          <Typography variant="body1">No spending data for the selected period.</Typography>
        </Box>
      ) : (
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={filteredAndAggregatedData}>
            ...
            <Tooltip formatter={(value: number, name: string, props: any) => [`$${value.toFixed(2)}`, props.payload.date]} />
            <Legend />
            <Line type="monotone" dataKey="amount" stroke="#8884d8" activeDot={{ r: 8 }} name="Spending" />
          </LineChart>
        </ResponsiveContainer>
      )}
    </Paper>
