...
export const SpendingByCategory = ({ finances }: { finances: FinanceEntity[] }) => {
  ...
  if (chartData.length === 0) {
    return (
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: 300 }}>
  ...
  const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#AA336A', '#FF69B4', '#A0522D'];
...
export const SubmissionStatusCounts = ({ finances }: { finances: FinanceEntity[] }) => {
  ...
  const statusConfig = {
    [Status.APPROVED]: { icon: <CheckCircleOutline />, color: 'green', label: 'Approved' },
    [Status.REJECTED]: { icon: <HighlightOff />, color: 'red', label: 'Rejected' },
    [Status.PENDING]: { icon: <HourglassEmpty />, color: 'orange', label: 'Pending' },
  };

  const statusesToDisplay = Object.keys(counts) as Status[];

  if (finances.length === 0) { // Check if finances array is empty, which means counts will be empty
    return (
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: 150 }}>
  ...
  return (
    <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', minHeight: 150 }}>
      ...
      {statusesToDisplay.map((status) => {
        ...
        const config = statusConfig[status as keyof typeof statusConfig] || { icon: <HelpOutline />, color: 'grey', label: status };

        return (
          <Box key={status} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            {React.cloneElement(config.icon, { sx: { color: config.color, mr: 1 } })}
            <Typography variant="body1" sx={{ color: config.color, fontWeight: 'medium' }}>
              {config.label}: <Typography component="span" variant="body1" sx={{ fontWeight: 'bold' }}>{count}</Typography>
            </Typography>
          </Box>
        );
      })}
